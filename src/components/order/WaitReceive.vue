		...
		<v-touch @swipeleft="onSwipeLeft()" @swiperight="onSwipeRight()">
			...
			<transition :name="slidename">
				<div class="container111">
					<div v-if="!havePage">
						<nopage style="margin-top:2rem"></nopage>
					</div>
					<div class="container_order" v-if="havePage">
...
<script>
	...
	export default {
		...
		methods: {
			getOrderList(){
				const that = this
				this.$http.get('/myapi/adel-shop/app/auth/orderList.htm?orderStatus=30')
					.then(function(res) {
						if (res.data.data != null) {
						
						console.log('res.data11111111111', res.data.data);
						...
						console.log('thiat.orderLuist',that.orderList);
					}
						else{
							console.log('没有数据');
							that.havePage = false
			...
			getOrderDetail() {
				this.$http.get('/myapi/adel-shop/app/auth/orderDetail.htm')
					.then(function(res) {
